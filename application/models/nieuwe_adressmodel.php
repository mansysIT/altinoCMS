...
class nieuwe_adressmodel
{
	...
	public function saveNewAdress()
	{
		...
		{	
			...
			$tel = $this->__params['POST']['tel'];
			$email = $this->__params['POST']['email'];
			$rekening = $this->__params['POST']['rekening'];
			$this->__db->execute("INSERT INTO bouw_adresy (
				city, adres, postcode, private_naam, private_achternaam, private_id_kaart, private_geboortedatum, bedrijf_bedrijf, bedrijf_adres, bedrijf_postcode, bedrijf_stad, 
				bedrijf_kvk, bedrijf_btw, tel, email, rekening) VALUES ('$city', '$adres', '$postcode' , '$private_naam' , '$private_achternaam' , '$private_id_kaart' , '$private_geboortedatum' ,
				 '$bedrijf_bedrijf' , '$bedrijf_adres' , '$bedrijf_postcode' , '$bedrijf_stad' , '$bedrijf_kvk' , '$bedrijf_btw' , '$tel' , '$email' , '$rekening')");
			//header("Location: ".SERVER_ADDRESS."administrator/nieuwe_adress/nieuwe_adress");
		}
	}
